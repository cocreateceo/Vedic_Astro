...
async function handler(event) {
  // Rewrite Next.js 16 RSC dot-separated prefetch paths to directory paths
  var _u = event.request.uri;
  if (_u.includes('__next.') && _u.endsWith('.txt')) {
    var _m = _u.match(/^(.+\/__next\.[^.]+)\.(__[^.]+__\.txt)$/);
    if (_m) { event.request.uri = _m[1] + '/' + _m[2]; }
  }
