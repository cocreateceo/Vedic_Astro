...
import { generateWeeklyContent } from './lib/horoscope-engine';
...
export async function handler() {
  ...
  const subscribers = await querySubscribers('weekly');
  console.log(`Weekly newsletter: ${subscribers.length} subscribers`);
  ...
  for (const sub of subscribers) {
    try {
      ...
      const content = generateWeeklyContent(moonSignIndex, today);
      content.name = name;
      content.unsubscribeUrl = buildUnsubscribeUrl(apiUrl, sub.email);
      content.dashboardUrl = DASHBOARD_URL;

      const { subject, html } = renderWeeklyEmail(content);
  ...
  console.log(`Weekly newsletter complete: ${sent} sent, ${errors} errors`);
